...
#     last_threevars = None
    ...
    async def post(self, y=y_start, m=m_start) -> Union[str, None]:
        ...
        async with self.ss.post(self.url_history, headers=self.headers, proxy=await self.proxy, data=payload) as r:
            print(r.status)
            if r.status == 200:
                print(await r.text())
                rtext = await r.text(encoding='utf8')
    ...
    async def get_ymdata(self, y=y_start, m=m_start) -> list:
        ...
        if rtext := await self.post(y=y, m=m):
            if self.nodata in rtext:
                self.ymdata.append(self.nodata)
            else:
                doc = pq(rtext, parser='html')
                #
    ...
    async def getdata_in_ymrange(self):
        ...
        for y, m in yms:
            ...
            # 抓到才下個月
            if self.nodata not in tmp:
                await self.bulk_insert()
            else:
                # 有可能查無資料
                print(f'{y}-{m}: {self.nodata}')
