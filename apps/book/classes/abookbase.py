...
import os
import itertools
from typing import Dict, Any, Awaitable, Tuple, Union
import random
...
class BOOKBASE(object, metaclass=VALIDATE):
    ...
    INFO_COLS = SimpleNamespace(**dict(zip(info_cols, info_cols)))
    #
    ...
    def __new__(cls, **init):
        '''
            ...
            各家store管理自己的objs及instance
            VALIDATE會增加各家class.objs，class_dict['objs'] = {}
        '''
        bid = init.get('bookid') or cls.info_default['bookid']
        # 沿用或new，VALIDATE會增加各家class.objs，class_dict['objs'] = {}
    ...
    async def proxy(self) -> Union[str, None]:
        ...
        if ippt := await ipscfg.ips_Queue.get():
            ...
            if proxy:
                # 失敗越多次，提高top_proxy的選取機率
                sample1 = [proxy] * (1 + self.update_errcnt)
                sample2 = [tmp] * (update_errcnt_max + 3 - self.update_errcnt)
                proxy = random.choice(sample1 + sample2)
            else:
                proxy = tmp
    ...
    @abstractmethod
    async def update_info(self, uid=None, proxy=None, db=dbwtb, proxy_none=False) -> Union[int, None, bool]:
        ...
        if uid == 1:
            if proxy_none:
                self.now_proxy = proxy  # 允許不用代理，接收None
            else:
    ...
    def price_sub(self, price: str) -> str:
        '''只保留0-9及小數點'''
        return re.sub('[^.0-9]', '', price)

    def price_list_handle(self, price_list: str) -> Union[str, int]:
        '''定價售價統一base處理，回傳: 空字串_int'''
        result: Union[str, int] = price_list
        #
        if price_list:
            price_list = self.price_sub(price_list)
            #
            if not re.match(self.int_pattern, price_list):
                result = self.int_err
            else:
                result = int(price_list)
        #
        return result

    def price_sale_handle(self, price_sale: str) -> Union[str, float, int]:
        '''定價售價統一base處理，回傳: 空字串_float_int'''
        result: Union[str, float, int] = price_sale
        #
        if price_sale:
            price_sale = self.price_sub(price_sale)
            #
            if not re.match(self.float_pattern, price_sale):
                result = self.float_err
            else:
                result = float(price_sale)
                if result == (tmp_int := int(result)):
                    result = tmp_int
        #
        return result

    def isbn1013(self, isbn: str, len_isbn: int) -> Tuple[Union[str, None], Union[str, None]]:
        '''決定isbn10或13'''
        isbn10 = (len_isbn == 10 and isbn) or None
        isbn13 = (len_isbn == 13 and isbn) or None
        return isbn10, isbn13

    def update_handle(self, locals_var: dict):
        '''爬成功的update統一base處理，紀錄空字串及None以外的值'''
    ...
    async def bid_Queue_put(cls, C, Q: asyncio.Queue):
        '''base對書號queue無窮put'''
        while 1:
            # if isinstance(C, collections.AsyncGenerator):  # 3.10會廢止
    ...
    @classmethod
    async def bid_update_loop(cls, bids: Union[list, set], DWU: int = days_without_update) -> bool:
